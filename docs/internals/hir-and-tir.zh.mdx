...
欢迎来到 HIR 与 TIR 篇章！这篇文章我们先回答两个问题：HIR 和 TIR 分别是什么，它们之间是什么关系。然后解释为什么需要分成两层而不是一步到位。之后分别展示 HIR 的构成和 TIR 的构成，用 Syl 代码示例说明从 AST 到 HIR 再到 TIR 的完整过程。最后讲 side table 模式的设计思想。
...
**定义（Def）。** 每个顶层声明是一个定义，比如 `module Top`、`fn add`、`const WIDTH`。每个定义有一个唯一的 `DefId`。
...
fn double(x: UInt<WIDTH>) -> UInt<WIDTH> {
    return x + x
}
...
| D0 | ConstItem | WIDTH |
| D1 | FnItem | double |
| D2 | CallableItem (Module) | Top |

`double` 函数的函数体包含一个表达式 `x + x`。这个表达式的结构在 HIR 中表示为：
